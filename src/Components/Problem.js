...
function Problem() {
  ...
  const filteredProblems =
    sortBy === "All"
      ? problems
      : problems.filter((problem) => problem.difficulty === sortBy);
      ...
      <ul className="space-y-4">
        {filteredProblems.map((problem) => (
          <li key={problem.id}>
            <div
              className={`bg-${
                isProblemSolved(problem.id) ? "green" : "white"
              } rounded-lg shadow-md p-4 flex justify-between items-center`}
            >
              <div>
              ...
              <button
                ...
                className={`ml-4 rounded-lg px-4 py-2 text-white font-bold ${
                  isProblemSolved(problem.id) ? "bg-green-500" : "bg-gray-700"
                }`}
              >
